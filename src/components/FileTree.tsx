...
function FileTreeNode({ node, depth, onFileSelect }: FileTreeNodeProps) {
  ...
  return (
    <div>
      <div
        className={cn(
          'flex items-center gap-1 py-0.5 px-2 hover:bg-accent/50 cursor-pointer text-sm font-mono',
          'transition-colors'
        )}
...
function FileIcon({ filename }: { filename: string }) {
  ...
  return (
    <div className={cn('w-4 h-4 flex items-center justify-center shrink-0', style.color)}>
      {style.text ? (
        <span className="text-[8px] font-bold">{style.text}</span>
      ) : (
        <svg
          viewBox="0 0 24 24"
          fill="none"
