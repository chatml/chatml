...
func NewRouter(ctx context.Context, s *store.SQLiteStore, hub *Hub, agentMgr *agent.Manager, ghClient *github.Client, linearClient *linear.Client, bw *branch.Watcher, prw *branch.PRWatcher, prCache *github.PRCache, issueCache *github.IssueCache, statsCache *SessionStatsCache, diffCache *DiffCache, snapshotCache *SnapshotCache, aiClient ai.Provider, scriptRunner *scripts.Runner, ollamaMgr *ollama.Manager, opts ...RouterOption) (http.Handler, *Handlers, func()) {
	...
	r.Route("/api/repos", func(r chi.Router) {
		...
		// File operation endpoints (create, rename, delete, duplicate, move, discard)
		r.Post("/{id}/sessions/{sessionId}/files/create", h.CreateFile)
		r.Post("/{id}/sessions/{sessionId}/folders/create", h.CreateFolder)
		r.Put("/{id}/sessions/{sessionId}/files/rename", h.RenameFile)
		r.Post("/{id}/sessions/{sessionId}/files/delete", h.DeleteFile)
		r.Post("/{id}/sessions/{sessionId}/files/duplicate", h.DuplicateFile)
		r.Post("/{id}/sessions/{sessionId}/files/move", h.MoveFile)
		r.Post("/{id}/sessions/{sessionId}/files/discard", h.DiscardChanges)
	...
	r.Route("/api/conversations", func(r chi.Router) {
		...
		r.Post("/{convId}/approve-batch-tools", h.ApproveBatchTools)
