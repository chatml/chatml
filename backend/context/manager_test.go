...
func TestManager_ShouldMicrocompact(t *testing.T) {
	...
	// Not enough tool results
	assert.False(t, m.ShouldMicrocompact(3, time.Minute))

	// Enough results and enough time
	assert.True(t, m.ShouldMicrocompact(10, time.Minute))
}
...
func TestManager_ShouldMicrocompact_TooSoon(t *testing.T) {
	...
	assert.False(t, m.ShouldMicrocompact(100, 5*time.Minute))
}
