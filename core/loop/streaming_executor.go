...
// Collect must be called exactly once, after all AddTool calls are complete.
// The returned map and slice are internal references — do not modify them.
//
...
func (se *StreamingToolExecutor) Collect(ctx context.Context) (concurrentResults map[string]tool.ToolCallResult, pendingSerial []provider.ToolUseBlock) {
	...
	// Release the derived context now that all tools are done
	se.siblingCancel()
