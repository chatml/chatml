...
// NOTE: ShouldExtract+Extract is not atomic — duplicate extraction goroutines are possible
// but harmless (just wasteful). The caller must pass a deep-cloned message slice.
...
func (sn *SessionNotes) GenerateNotes(ctx context.Context, messages []provider.Message) (string, error) {
	...
	for event := range stream {
		...
		if event.Type == provider.EventError && event.Error != nil {
			...
			return "", fmt.Errorf("session notes stream error: %w", event.Error)
		}
	}
