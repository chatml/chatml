...
// sanitizeErrorToolResults trims whitespace from error tool_result blocks to
// normalize their content. The API rejects rich content in error results,
...
func ensureToolResultPairing(messages []provider.Message) []provider.Message {
	...
	for i := 0; i < len(messages); i++ {
		...
		// mutating the original. SAFETY: This function is only called from
		// normalizeMessages which passes a cloned slice. Do NOT call this
		// function directly on the live r.messages slice.
		if len(toolUseIDs) > 0 {
...
func applyToolResultBudget(messages []provider.Message, maxBytes int) []provider.Message {
	...
	for i := range messages {
		...
		for j := range messages[i].Content {
			...
			// NOTE: This may double-truncate results that were already per-result truncated above.
			// The truncation message replaces the original, which is cosmetic but loses the original byte count.
