...
type Provider interface {
	...
	// PrewarmConnection initiates a TCP+TLS handshake to the API endpoint
	// in the background so the next StreamChat call can reuse the warm connection.
	// This is called while tools are executing to overlap network setup with computation.
	// Implementations that don't support pre-warming should no-op.
	PrewarmConnection()
}
...
type ChatRequest struct {
	...
	ServerTools    []ServerToolDef `json:"-"`                         // Server-managed tools (serialized per-provider)
}
...
type Capabilities struct {
	...
	SupportsNativeSearch  bool // Server-side web search (Anthropic)
}

// ServerToolDef defines a server-managed tool (e.g., web search) that the API
// executes on our behalf. Unlike regular tools, the provider handles execution.
