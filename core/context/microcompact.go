...
//
// WARNING: This function mutates the input slice's content blocks in place.
// Callers must pass a cloned slice if the original must be preserved.
//
...
func Microcompact(messages []provider.Message, keepRecent int) []provider.Message {
	...
	for idx, pos := range positions {
		...
		// NOTE: This mutates the input slice's content blocks in place.
		// Callers must pass a cloned slice if the original must be preserved.
