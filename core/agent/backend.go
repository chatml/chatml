...
// ToolApprovalOverride represents a per-tool decision in a batch approval response.
...
type ConversationBackend interface {
	...
	SendMessageWithAttachments(content string, attachments []core.Attachment) error
	...
	StoreOrDeferMessage(msg *core.Message) bool

	// EndTurnAndTakePending clears active turn and returns any deferred message.
	EndTurnAndTakePending() *core.Message
	...
	// perTool maps tool use IDs to individual ApprovalResponse overrides (action, specifier, updatedInput).
...
// using the native Go agentic loop. The apiKey and oauthToken are passed so
// the factory can select and authenticate a provider without importing backend
// packages.
