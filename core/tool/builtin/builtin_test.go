...
func TestWriteTool_LineCount(t *testing.T) {
	...
	require.NoError(t, err)
	// "a\nb\nc\n" has 3 lines (trailing newline does not add a line)
	assert.Contains(t, result.Content, "3 lines")
}
...
func TestRegisterAll(t *testing.T) {
	...
	assert.NotNil(t, reg.Get("NotebookEdit"))
