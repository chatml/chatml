...
type PlanModeCallback interface {
	...
	// PrePlanPermissionMode returns the permission mode that was active before
	// plan mode was entered, so ExitPlanMode can restore it correctly.
...
func (t *ExitPlanModeTool) Execute(ctx context.Context, input json.RawMessage) (*tool.Result, error) {
	...
	case result := <-respCh:
		if result.Approved {
			restoreMode := t.callback.PrePlanPermissionMode()
			if restoreMode == "" {
				restoreMode = "default"
			}
			t.callback.SetPermissionModeDirect(restoreMode)
