...
func (t *WebFetchTool) Execute(ctx context.Context, input json.RawMessage) (*tool.Result, error) {
	...
	// NOTE: The pre-flight validateNotPrivateHost check is redundant with this DialContext
	// hook but provides earlier error reporting. The DialContext hook is the real SSRF defense
	// and also covers redirect targets.
...
// This covers cloud metadata endpoints (169.254.169.254 is link-local, fd00:ec2::254 is private)
// and prevents SSRF via 0.0.0.0 / :: (which route to localhost on most systems).
...
// Pre-compiled regexes for HTML stripping (avoid per-call compilation).
var (
