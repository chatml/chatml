...
func (e *Executor) Execute(ctx context.Context, calls []ToolCall) []ToolCallResult {
	...
	// Run concurrent tools first (in parallel, semaphore-bounded),
	// then run serial tools sequentially after all concurrent tools finish.
	// This prevents data races between read and write tools targeting the same files.
	if len(concurrent) > 0 {
	...
	// Serial tools run sequentially after concurrent tools complete
