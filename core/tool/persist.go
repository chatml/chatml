...
func (p *ResultPersister) MaybePersist(toolUseID, content string) (inline string, persisted bool) {
	...
	// Build preview (rune-safe truncation to avoid splitting multi-byte characters)
	preview := content
	if len(preview) > p.previewLen {
		preview = preview[:p.previewLen]
		// Trim any trailing incomplete UTF-8 sequence
		for len(preview) > 0 && preview[len(preview)-1]&0x80 != 0 {
			r, size := utf8.DecodeLastRuneInString(preview)
